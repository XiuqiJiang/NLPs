...
def get_esm_embeddings(
    ...
    for i in range(0, len(sequences), batch_size):
        ...
        with torch.no_grad():
            ...
            # 验证batch_embeddings的形状
            expected_shape = (len(batch_sequences), model.config.hidden_size)
            if batch_embeddings.shape != expected_shape:
                raise ValueError(
                    f"Embeddings形状错误: 期望 {expected_shape}, 实际 {batch_embeddings.shape}\n"
                    f"这可能表明模型输出或池化过程有问题"
                )
    ...
    if embeddings.ndim != 2:
        raise ValueError(f"最终embeddings维度错误: 期望2维 [N, embed_dim], 实际 {embeddings.ndim}维")
    if embeddings.shape[0] != len(sequences):
        raise ValueError(f"embeddings数量不匹配: 期望 {len(sequences)}, 实际 {embeddings.shape[0]}")
